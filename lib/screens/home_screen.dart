...
class _HomeScreenState extends State<HomeScreen> {
  ...
  /// Pre-populates UI from local cache immediately so data shows even offline.
  Future<void> _loadCachedData() async {
    final cached = await CacheService.getUserData();
    if (cached != null && mounted) {
      setState(() {
        _points = (cached['points'] as num?)?.toInt() ?? 0;
        _streak = (cached['streak'] as num?)?.toInt() ?? 0;
        _userName = cached['name'] as String? ?? 'User';
        _activeTheme = cached['activeTheme'] as String? ?? 'Dark Mode';
        _activeFrame = cached['activeFrame'] as String? ?? 'Default';
        _unlockedThemes = List<String>.from(cached['unlockedThemes'] ?? ['Dark Mode']);
        _unlockedFrames = List<String>.from(cached['unlockedFrames'] ?? ['Default']);
      });
    }
    final cachedLeaderboard = await CacheService.getLeaderboard();
    if (cachedLeaderboard != null && mounted) {
      setState(() {
        _leaderboard = cachedLeaderboard.asMap().entries.map((e) {
          return LeaderboardItem(
            rank: e.key + 1,
            name: e.value['name'] as String? ?? 'Player',
            points: (e.value['points'] as num?)?.toInt() ?? 0,
          );
        }).toList();
      });
    }
    final cachedAnnouncement = await CacheService.getAnnouncement();
    if (cachedAnnouncement != null && mounted) {
      setState(() => _announcementText = cachedAnnouncement);
    }
  }
    ...
    _announcementSubscription = FirebaseFirestore.instance
        ...
        .listen((snap) {
          if (snap.exists && mounted) {
            ...
            CacheService.saveAnnouncement(text);
          } else if (mounted) {
            setState(() => _announcementText = null);
            CacheService.saveAnnouncement(null);
          }
        }, onError: (e) {
        ...
        .listen(
          (snap) {
            ...
            if (_lastPoints != null && newPoints > _lastPoints!) {
              ...
              if (mounted) {
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    content: Text('🎉 Recycle complete! +$delta point${delta == 1 ? '' : 's'} added to your account.'),
            ...
            if (mounted) {
              ...
              // Save to local cache every time Firestore updates
              CacheService.saveUserData({
                'points': newPoints,
                'streak': streak,
                'activeTheme': theme,
                'activeFrame': frame,
                'unlockedThemes': uThemes,
                'unlockedFrames': uFrames,
                'name': name,
              });
            }
          },
        ...
        .listen(
          (snap) {
            ...
            if (mounted) {
              ...
              // Cache the leaderboard for offline display
              CacheService.saveLeaderboard(
                list.map((e) => {'name': e.name, 'points': e.points}).toList(),
              );
            }
          },
          onError: (e) {
            if (!mounted) return;
            setState(() {
              _leaderboardError = 'Failed to load leaderboard: ${e.toString()}';
              _isOffline = true;
            });
          },
        );
  ...
  Future<void> _logout() async {
    await CacheService.clearAll(); // Clear local cache on logout
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          FilledButton.icon(
            onPressed: _isOffline ? null : () => Navigator.of(context).push(
              MaterialPageRoute(builder: (_) => const ConnectScreen()),
            ),
            icon: Icon(_isOffline ? Icons.wifi_off : Icons.qr_code_scanner),
            label: Text(_isOffline ? 'Unavailable Offline' : 'Connect to Smart Bin'),
          ...
          ..._leaderboard.map((e) {
            ...
            if (e.rank == 1) {
              bgColor = Colors.amber.shade900.withValues(alpha: 0.2);
              rankColor = Colors.amber;
              trailingWidget = const Icon(Icons.star, color: Colors.amber);
            } else if (e.rank == 2) {
              bgColor = Colors.grey.shade400.withValues(alpha: 0.2);
            ...
            return Card(
              ...
              shape: RoundedRectangleBorder(
                borderRadius: BorderRadius.circular(12),
                side: e.rank <= 3 ? BorderSide(color: rankColor.withValues(alpha: 0.5), width: 1) : BorderSide.none,
              ),
              child: ListTile(
              ...
              decoration: BoxDecoration(
                ...
                gradient: _activeFrame == 'Default' ? null : LinearGradient(
                  colors: [
                    ...
                    _getFrameColor(_activeFrame).withValues(alpha: 0.5),
                  ],
